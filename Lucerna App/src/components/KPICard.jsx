...
export default function KPICard({
  ...
  return (
    <div
      ...
      className={`h-full rounded-xl border bg-[var(--bg-card-solid)] backdrop-blur-2xl p-5 transition-all duration-500 ease-out
        ${active
        ...
        <p className="text-sm text-[var(--text-secondary)] font-medium flex items-center gap-1.5 leading-tight min-h-[2.25rem]">
          ...
          {anomaly && (
            <span
              ...
              title={`${anomaly.isPositive ? "Up" : "Down"} ${Math.abs(anomaly.change)}% vs prior${anomaly.isGood ? "" : " — investigate"}`}
              aria-label={`Anomaly: ${anomaly.isPositive ? "up" : "down"} ${Math.abs(anomaly.change)}% vs prior period`}
            >
              <span
                className={`absolute inset-0 rounded-full animate-ping ${
                  anomaly.isGood ? "bg-[var(--success)]" : "bg-[var(--warning)]"
                } opacity-60`}
              />
              <span
